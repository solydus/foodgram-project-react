...
class FavoriteViewSet(viewsets.ModelViewSet):
    ...
    def delete(self, request, recipe_id):
        ...
        if not Favorite.objects.filter(
            recipe=recipe, recipe_lover=recipe_lover
        ).exists():
            return Response(
        ...
        get_object_or_404(
            Favorite, recipe_lover=recipe_lover, recipe=recipe).delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class ShoppingCartViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        recipe = get_object_or_404(Recipe, pk=self.kwargs.get("recipe_id"))
        serializer.save(recipe=recipe, cart_owner=self.request.user)

    @action(methods=["delete"], detail=True)
    def delete(self, request, recipe_id):
        recipe = get_object_or_404(Recipe, pk=recipe_id)
        if not ShoppingCart.objects.filter(
            recipe=recipe, cart_owner=self.request.user
        ).exists():
            return Response(
...
class DownloadShoppingCart(APIView):
    ...
    def get(self, request):
        ...
        if shopping_list is None:
            return Response(
                {"errors": "В вашем списке покупок ничего нет"},
                status=status.HTTP_400_BAD_REQUEST,
            )
